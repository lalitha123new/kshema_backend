...
import com.screener.app.dao.UserSessionDAOImpl;
...
public class JWTUtil {
	...
	public String createUserToken(String sesId, String sessionToken, users objUser) throws IllegalArgumentException, NoSuchAlgorithmException,
		...
		try {
			...
			token = JWT.create().withIssuer(LoadConfig.getConfigValue("JWT_ISSUER_KEY")).withIssuedAt(new Date())
					...
					.withClaim("userId", objUser.getUsers_id())
					.withClaim("username", objUser.getUser_name())
					.withClaim("role", objUser.getRole())
					.withClaim("status", objUser.getEmail())
